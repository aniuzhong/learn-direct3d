...
void CollapseRowPitch(unsigned* data, int width, int height, int pitch);    // assumes pitch is measured in 32-bit texels, not bytes; use locked_rect.Pitch >> 2
void Compress32To24(byte* data, int width, int height);    // bpp is bits, not bytes
void WriteDataToTGA(const char* name, void* data, int width, int height, int bpp);
...
void RenderFrame(void)
{
    ...
    CollapseRowPitch((unsigned int*)locked_rect.pBits, desc.Width, desc.Height, locked_rect.Pitch >> 2);

    Compress32To24((byte*)locked_rect.pBits, desc.Width, desc.Height);

    std::string strIndex = std::to_string(index);
    std::string strPath = "D:/" + strIndex + ".tga";
    WriteDataToTGA(strPath.c_str(), locked_rect.pBits, desc.Width, desc.Height, 24);
